...
export class Iterator<T> {
   ...
   */
  public static forArray<T>(items: T[], reverse = false): Iterator<T> {
   ...
   */
  public static forObject<T>(items: { [key: string]: T }, hasOwnProperty = true): Iterator<T> {
    return new Iterator<T>(iterator => {
  ...
  public reverse(): Iterator<T> {
    return new Iterator<T>(iterator => {
      ...
      let modifies = false
      const actions: IteratorAction[] = []
      const replaces: T[] = []

      for (let i = items.length - 1; i >= 0; i--) {
        const item: T = items[i]
        const action: IteratorAction = iterator.act(item)
      ...
      if (modifies) {
        ...
        this.iterate((item, it) => {
          switch (actions[index]) {
            case IteratorAction.Remove:
              it.remove()
              break
            case IteratorAction.Replace:
              it.replace(replaces[index])
              break
          }
   ...
   */
  public map<W>(
    mapper: IteratorCallback<T, W>,
    filter: IteratorFilter<T> = null,
    unmapper: (replaceWith: W, current: W, item: T) => T = null
  ): Iterator<W> {
