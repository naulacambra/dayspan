...
export class Day {
  ...
  public prev(days = 1): Day {
    return this.relativeDays(-days)
  }

  public next(days = 1): Day {
    return this.relativeDays(days)
  }
  ...
  public prevMonth(months = 1): Day {
    return this.relativeMonths(-months)
  }

  public nextMonth(months = 1): Day {
    return this.relativeMonths(months)
  }
  ...
  public prevWeek(weeks = 1): Day {
    return this.relativeWeeks(-weeks)
  }

  public nextWeek(weeks = 1): Day {
    return this.relativeWeeks(weeks)
  }
  ...
  public prevYear(years = 1): Day {
    return this.relativeYears(-years)
  }

  public nextYear(years = 1): Day {
    return this.relativeYears(years)
  }
  ...
  public prevHour(hours = 1): Day {
    return this.relativeHours(-hours)
  }

  public nextHour(hours = 1): Day {
    return this.relativeHours(hours)
  }
  ...
  public withTimes(
    ...
    millisecond: number = Constants.MILLIS_MIN
  ): Day {
  ...
  public end(inclusive = true): Day {
    return inclusive ? this.mutate(d => d.endOf('day')) : this.mutate(d => d.startOf('day').add(1, 'day'))
  }

  public isEnd(): boolean {
    return (
      this.hour === Constants.HOUR_MAX &&
  ...
  public endOfHour(inclusive = true): Day {
    return inclusive ? this.mutate(d => d.endOf('hour')) : this.mutate(d => d.startOf('hour').add(1, 'hour'))
  }
  ...
  public endOfWeek(inclusive = true): Day {
    return inclusive ? this.mutate(d => d.endOf('week')) : this.mutate(d => d.startOf('week').add(1, 'week'))
  }
  ...
  public endOfMonth(inclusive = true): Day {
    return inclusive ? this.mutate(d => d.endOf('month')) : this.mutate(d => d.startOf('month').add(1, 'month'))
  }
  ...
  public endOfYear(inclusive = true): Day {
    return inclusive ? this.mutate(d => d.endOf('year')) : this.mutate(d => d.startOf('year').add(1, 'year'))
  }
  ...
  public toISOString(keepOffset = false): string {
  ...
  public static fromDayIdentifier(id: number): Day {
    const date: number = id % 100
    const month: number = Math.floor(id / 100) % 100 - 1
    const year: number = Math.floor(id / 10000)
  ...
  public static getLastWeekspanOfYear(date: moment.Moment): number {
    const lastOfYear = date.clone().endOf('year')
    const daysInYear: number = lastOfYear.dayOfYear()

    return Math.floor((daysInYear - date.dayOfYear()) / Constants.DAYS_IN_WEEK)
  }

  public static getWeekOfYear(date: moment.Moment): number {
    const firstOfYear = date.clone().startOf('year')
    const weeks: number = date.week()
  ...
  public static getLastFullWeekOfYear(date: moment.Moment): number {
    const firstOfYear = date.clone().startOf('year')
    const weeks: number = date.week()
    const weeksMax: number = date.weeksInYear()
    const lastWeek: number = weeksMax - weeks
  ...
  public static getLastFullWeekOfMonth(date: moment.Moment): number {
    return Math.floor(
      (date.daysInMonth() - date.date() - (Constants.WEEKDAY_MAX - date.day()) + Constants.DAYS_IN_WEEK) /
